...
- DANGER flag / 5-match graduation (1.6): a no-data player's rating builds from their own real performance as they play (§1.1–1.5), not from manual external backfill. While a player has fewer than 5 real matches on record, they carry a DANGER flag wherever their rating is shown — in the rollout's flagging above, and on Ratings H2H (Phase 4). At 5 matches the flag drops permanently. DANGER redistribution formula: opponent's old rating is the base (not the DANGER player's own, not yet meaningful) — Clear Win/Win add ¾/⅓ of that base, Clear Loss/Loss subtract the same fractions, Tight Win/Tight Loss get no special correction beyond normal accrual. This rollout's output is what Phase 4 reads from; Phase 4 cannot function correctly until this has actually been run.
