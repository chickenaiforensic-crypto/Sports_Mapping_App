...
- Calibration Rollout (1.10): the actual deliverable for this phase is not a single-edition demo — it's a year-by-year calibration run, controlled by the mute checkboxes (not hardcoded to 2022), each year using the prior year's calibrated output as its incoming source. Confirmed operational start point: 2023, with 2022 as the pure baseline/source year (never itself calibrated, only real raw data). This is a pre-launch data-preparation pass run once by the team, not a live per-visit feature. No-data fallback hits (§1.6) must be explicitly flagged during the run as DANGER/UNTESTED status — name, tour, year — not silently defaulted and passed over.
- Undo ordering (1.10): calibration can only be undone from the highest (most recent) calibrated year backward, one year at a time, in strict reverse order — never from a middle year in isolation. Every year's calibration depends on the prior year's calibrated output as its incoming source, so removing a middle year while later years still reference it produces an inconsistent state. The UI must not offer to undo a year while any later year is still calibrated.
...
| v1.12 | August 2026 | Matches spec v1.25. Calibration start point moved to 2023 (2022 baseline), now that UNTESTED handling removes the risk that kept it at 2024. Added the Undo Ordering requirement to Phase 2: the UI must structurally prevent undoing a calibrated year while any later year is still calibrated — highest-year-backward only, never a middle year in isolation.
