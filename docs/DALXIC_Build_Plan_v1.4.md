...
- Muting (1.8): once a round/event's real data has calibrated that event's ratings, the real data is muted. Confirmed: reversible, scoped per edition and per year, and remains available as reference data (pickers, future systems) even while muted — only excluded from re-triggering calibration on a re-run.
...
Completion audit must specifically check: Backtest vocabulary stays Correct / Wrong / Tie with Wrong sub-split into Clear Wrong / Wrong / Flippable Wrong — never "Loss," which belongs to Phase 4 only. Also confirm muting matches spec Section 1.8 exactly: reversible, per-edition/per-year granularity, muted data still referenceable elsewhere.
