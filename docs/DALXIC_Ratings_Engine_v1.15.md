...
Source event selection, confirmed: when a player has more than one prior event available as a possible source, the single best-performing event is used — never the most recent by date, and never combined or summed across multiple events. "Best-performing" means the event that produced the highest rating gain for that player. This is the same rule used later for post-calibration consolidation (Section 1.9) — one consistent principle for picking a single representative result out of several, applied both going in and coming out of calibration.
...
Scope note: Ratings H2H is the only compute system being built, documented, or phased right now. Other systems referenced in earlier builds (Lookup, Leaderboard, Tournament Performance, Common Opponents, etc.) are explicitly out of scope until Ratings H2H is complete and working correctly. They are not specified in this document and no build work should start on them ahead of that.
