...
DANGER player redistribution — while a player is still DANGER-flagged, their own prior rating isn't yet a meaningful base for the standard redistribution table (Section 1.6), so it's replaced with their opponent's old rating as the base to build from or fall from. This mirrors the same category-based structure already established, applied in both directions:

| Category | DANGER player's new rating |
|---|---|
| Clear Win | opponent_old + ¾ × opponent_old (= 1.75 × opponent_old) |
| Win | opponent_old + ⅓ × opponent_old (= 1.33 × opponent_old) |
| Tight Win | no special correction — accrual only, same as the existing rule |
| Clear Loss | opponent_old − ¾ × opponent_old (= 0.25 × opponent_old) |
| Loss | opponent_old − ⅓ × opponent_old (= 0.67 × opponent_old) |
| Tight Loss | no special correction — accrual only, same as the existing rule |

Worked example: a DANGER player loses to an opponent rated 50, a Clear Loss. New rating = 50 − (0.75 × 50) = 12.5. A DANGER player who instead beat that same opponent, Clear Win, would land at 50 + (0.75 × 50) = 87.5.

This does not replace normal margin-share accrual (Section 1.5), which still runs on every match regardless of category, DANGER-flagged or not — it replaces only the standard old-rating-based redistribution formula for as long as the player's own old rating isn't yet meaningful. Once 5 real matches accumulate, the player graduates to the standard table using their own real rating, exactly like any established player.
