...
The Match Points Record — this conversion happens exactly once per match, here at the System Language layer, and is stored as a canonical structured record: for every set, both players' points (after the reduction table above), the running total per player, and the match budget (Section 1.2). This record is generated once, per match, and referenced by every downstream layer — the Set Normaliser, score classification, the Ratings mapper, the Backtest — never re-derived independently at each stage from the raw score string.

This matters beyond convenience: every downstream number (margin share, Clear Win/Win/Tight Win classification, the redistribution table's inputs) is a function of this one record. Computing it once and referencing it everywhere is what keeps those numbers consistent with each other. Deriving margin share in one place and classification in another, each re-parsing the raw score independently, is exactly how they can silently drift out of agreement with each other.

Worked example — pA d. pB, 6-2 6-3:

| Set | pA points | pB points |
|---|---|---|
| 1 | 10 | 2 |
| 2 | 10 | 4 |

Match totals: pA 20, pB 6. Budget: 20 (2 sets × 10). From this one record: margin share (Section 1.3) = (20−6)/20 = 0.7. Score class (Section 1.6) = pB's 6 points is 30% of budget = Clear Win. Both derived from the same stored numbers, not recomputed separately from the score string twice.
