...
| v1.22 | August 2026 | Specified the exact DANGER player redistribution formula (Section 1.6): while DANGER-flagged, a player's own old rating isn't a meaningful base, so it's replaced with the opponent's old rating — Clear Win/Win add ¾/⅓ of that base, Clear Loss/Loss subtract the same fractions, Tight Win/Tight Loss get no special correction (accrual only, matching the existing rule). This runs alongside normal margin-share accrual, which is unaffected and continues on every match regardless of category. |
| v1.23 | August 2026 | Superseded v1.22's symmetric loss formula, which had a real failure mode: an untested player losing early to a highly-rated opponent (a rookie drawn against a top seed) could inherit an artificially high rating from that single loss. Introduced UNTESTED as a distinct status from a rating of 0 — no number exists at all, not a low-but-real one. While untested, losses cause no rating change whatsoever; only a first win establishes a real baseline, using the standard Win-category formula against the opponent's old rating. An established player beating a still-untested opponent earns a halved category addon (⅜/⅙ of their own old rating for Clear Win/Win, since there's no opponent rating to draw the normal fraction from) — a smaller reward reflecting that beating an unproven opponent carries less calibration value. Section 3.2 updated: UNTESTED blocks a real prediction entirely ("cannot compute"), which is a stronger condition than DANGER (fewer than 5 matches, which still produces a call, just flagged as provisional). Explicitly scoped: this entire mechanic lives in the Calibration Rollout (1.6/1.10); Ratings H2H never computes it, only reads the result. |
| v1.24 | August 2026 | Corrected Section 2.1 after an observed build issue — the System Update Request appeared to contain player/tournament claims that weren't independently verifiable. Two gaps closed: (1) the spec never explicitly required this output to be mechanically assembled from real stored records rather than generated by any AI process inside the app — now stated directly, with the request step itself required to contain zero unverified claims. (2) The request is now explicitly tournament-scoped only, never player-scoped — the system tracks tournaments as the shell holding data together; player data is only ever a byproduct of tournament data, never independently requestable. Each tournament's request line states its identity, last known real data point, and the threshold date (today) fresh data must reach; a tournament is only included if it's in process or completed relative to today's real date, never one that hasn't started yet.
| v1.25 | August 2026 | Calibration start point moved from 2024 to 2023, now that Section 1.6's DANGER/UNTESTED mechanism (v1.21–1.23) removes the risk that motivated staying at 2024 — an untested player can no longer be inflated by an early loss, so the higher untested-player count at an earlier start (92 vs. 79, confirmed via direct data check) is no longer a problem. 2022 is now explicitly stated as the pure baseline year, never itself calibrated; 2023 is the first calibrated year. Added the Undo Ordering rule: calibration can only be undone from the most recent calibrated year backward, one year at a time — never a middle year in isolation, since every later year depends on the one before it as its incoming source. The UI must enforce this structurally, not just validate against it. Also corrected a stale reference in the No-Data Flagging paragraph, which still described the manual-backfill process superseded back in v1.21 — updated to reflect the DANGER/UNTESTED mechanism as the actual current behavior.
| v1.26 | August 2026 | Closed two gaps a build audit surfaced — both honestly disclosed as unspecified in the code's own comments rather than silently guessed at. (1) Established player loses to a still-UNTESTED opponent: treated symmetrically with the winner's halved-addon rule — same category (Clear Win/Win/Tight Win, from the real result), same halved fractions (⅜/⅙), applied as a subtraction from the established player's own old rating. Both this and the untested winner's baseline formula reference the same pre-match snapshot of the established player's rating, computed once, not sequentially. (2) Two UNTESTED players meeting: nothing happens, regardless of outcome or category — both remain UNTESTED. Establishing a baseline requires a rated opponent to draw from; two untested players provide no such reference for each other. This replaces an earlier build's default behavior (winner establishing at exactly 0), which was a reasonable guess but not the intended rule.
| v1.27 | August 2026 | Added the Match Points Record to Section 1.1: the games-to-points conversion happens exactly once per match at the System Language layer and is stored as a canonical structured record (per-set points for both players, running totals, budget) — never re-derived independently by each downstream layer. This closes a real defect found by hand-tracing a real calibration run: the redistribution formula had been applying category fractions (¾, ⅓) flatly, without weighting by the match's actual real margin share, which is exactly the kind of drift this record's existence is meant to prevent. A 6-0 6-0 Clear Win and a 6-4 6-4 Clear Win must not receive the same addon — the addon should scale with how dominant the real result actually was, read from this one record, not a flat category constant.
| v1.28 | August 2026 | Corrected v1.27's Match Points Record — it used raw 0–10 point totals; the actual intended representation is a converted rating per set (points ÷ 10, a 0–1 scale) summed across the match, kept separately per player rather than netted into one signed number. This is the record that replaces the raw score for every internal purpose: classification reads the loser's summed rating as a share of set count, margin share (§1.3) is the plain subtraction between the two players' summed ratings, and Common Opponents (§1.11.1) reads its differential the same way — each player's rating minus the shared opponent's rating in their respective match, no reparsing needed.
...
The Match Points Record — this conversion happens exactly once per match, here at the System Language layer, and is stored as a canonical structured record. Every downstream layer — the Set Normaliser, score classification, the Ratings mapper, the Backtest, and any future Mapping System — reads from this one record. Never re-derived independently at each stage from the raw score string; that's exactly how numbers silently drift out of agreement with each other.

The record's core value is a converted rating, per set, per player: each set's points (from the table above) divided by 10, giving a 0–1 value per set. These per-set values are summed across the match to give each player their own match rating — not a single net margin, both players' own numbers, kept separately.

Worked example — Alcaraz d. Chardy, 6-0 6-2 7-5 (reduces to 6-0 6-2 6-4):

| Set | Alcaraz points | Chardy points | Alcaraz rating | Chardy rating |
|---|---|---|---|---|
| 1 | 10 | 0 | 1.00 | 0.00 |
| 2 | 10 | 2 | 1.00 | 0.20 |
| 3 | 10 | 4 | 1.00 | 0.40 |

Summed match rating: Alcaraz 3.00, Chardy 0.60. This pair of numbers is the record — it replaces the raw score for every internal purpose from here forward.

Everything downstream reads from this same pair, nothing recomputed separately:

- Score classification (Section 1.6): the loser's rating as a share of the match's set count (their budget, now on this scale) — 0.60 ÷ 3 sets = 20% → Clear Win. Same thresholds as before (≤50% Clear Win, 51–75% Win, ≥76% Tight Win), same proportions, just read off the rescaled numbers.
- Margin share (Section 1.3) is the signed difference between the two: Alcaraz 3.00 − Chardy 0.60 = +2.40. It is derived from this record, not computed separately.
- Common Opponents (Section 1.11.1, concept-level): pA's differential against a shared opponent X, and pB's differential against the same X, are each just this same subtraction — pA's rating minus X's rating in their match, pB's rating minus X's rating in theirs. Two clean numbers, straight from records that already exist, no reparsing needed to compare them.
...
DANGER status and the 5-match graduation threshold — once a player has an established rating (see Untested Status below for how that first happens), they are not held at a flat, unchanging number waiting on external backfill. Their rating builds from their own real performance via the same points and margin-share mechanism used for every player (Sections 1.1–1.5).
...
Untested status — not a rating of zero. Before a player has ever won a real tracked match, they are UNTESTED. This is a status, not a number: there is no rating to add to, subtract from, or compare — a rating of exactly 0 (Section 1.6, above) is still a real, comparable value, but UNTESTED means no value exists yet at all. The two must never be conflated.

This entire mechanic belongs to the Calibration Rollout (Sections 1.6/1.10) — the backend process that builds ratings from real results. It is separate from Ratings H2H (Section 3.2), which only reads and compares already-established ratings and never runs redistribution logic itself.

While a player is UNTESTED, losses do not change anything. No rating is created or altered by a loss — the player remains UNTESTED regardless of how many times they lose. This is a deliberate fix to an earlier, since-superseded version of this rule (v1.22), which had an untested player inherit and discount their opponent's rating on a loss — meaning a single early loss to a highly-rated player (a rookie drawing a top seed in an early round, for instance) could hand them an artificially high number despite having no real result to justify it. Removing any rating movement on a loss removes that failure mode entirely.

An untested player's first win is what establishes their real baseline rating — using the standard Win-category formula, exactly as already specified for an established player, with the winning opponent's old rating as the base: Clear Win = opponent_old + ¾ × opponent_old, Win = opponent_old + ⅓ × opponent_old, Tight Win = no correction. From that first win onward, the player has a real rating and is no longer UNTESTED — though they remain DANGER-flagged until their total real match count reaches 5, same as any newly-rated player.

When an established player beats a still-UNTESTED opponent, there is no opponent rating to draw the normal category fraction from, so the fraction is halved and applied to the winner's own old rating instead:

| Category | Established winner's addon (vs. an UNTESTED opponent) |
|---|---|
| Clear Win | + ⅜ × winner_old (half of the normal ¾) |
| Win | + ⅙ × winner_old (half of the normal ⅓) |
| Tight Win | no special correction — same as always |

This compensates the winner fairly for a genuine win while reflecting that beating an unproven opponent carries less calibration value than beating an established one — a smaller reward than the same category win would earn against a rated opponent.

The established loser's side of this same match — an established player who loses to a still-UNTESTED opponent is treated symmetrically: the same halved fraction, using the same category classification (from the real match result, Clear Win/Win/Tight Win as always), applied as a subtraction from the established player's own old rating rather than an addition:

| Category | Established loser's new rating (vs. a now-UNTESTED-no-longer opponent) |
|---|---|
| Clear Win | old rating − ⅜ × old rating (half of the normal ¾) |
| Win | old rating − ⅙ × old rating (half of the normal ⅓) |
| Tight Win | no special correction — same as always |

Both this and the untested winner's baseline-establishment formula (above) reference the established player's same pre-match old rating — computed from one snapshot, not sequentially, so the established player's loss-side reduction is never calculated from an already-reduced number, and the untested player's new baseline is never calculated from anything but the established player's true pre-match rating.

Both players UNTESTED — nothing happens. When two players with no established rating meet, regardless of the real match's outcome or category, neither player's status changes: both remain UNTESTED after the match. Establishing a baseline requires a real rated opponent to draw from; two untested players meeting each other provides no such reference point for either side. Only when an untested player subsequently meets an already-rated opponent does the baseline-establishment mechanism (above) apply.

On Ratings H2H (Section 3.2): if either selected player is UNTESTED, the page cannot compute a genuine prediction and must state this plainly — "cannot compute, [player] is untested" — rather than fabricate a call from a number that doesn't exist. This is a stronger condition than the DANGER flag: DANGER marks a real, if young, rating as provisional and still produces a call; UNTESTED means there is nothing there to compare at all, and no call should be produced.
...
The operational rollout: calibration runs year by year. 2022 serves as the pure baseline/source year — real raw data, never itself calibrated. 2023 is the first calibrated year, using 2022 as its incoming source, and the rollout proceeds forward one year at a time through to the present. Each year's Backtest pass uses the previous year's already-calibrated ratings as its incoming source (Section 1.6), calibrates every tournament in the current year against real results, and Section 1.9 consolidates each player down to their single best result for that year before the next year's pass begins.
...
Undo ordering — calibration can only be undone from the highest (most recent) calibrated year backward, one year at a time, in strict reverse order. A middle year can never be undone in isolation while a later year is still calibrated: every year's calibration depends on the prior year's calibrated output as its incoming source, so removing a middle year out of order leaves every later year referencing a baseline that no longer exists, producing an inconsistent, unrecoverable-without-a-full-redo state. The UI enforces this structurally — it does not offer to undo a year while any later year remains calibrated, the same way the mute checkboxes enforce contiguity structurally (Section 1.8) rather than merely validating against a bad state after the fact.

No-data flagging — every time the rollout (or any Mapping System calibration built on the same mechanism) encounters a player with no established rating, that player must be explicitly flagged as UNTESTED — logged with the player's name, tour, and the year they were being calibrated for — not silently processed and passed over. This is not a trigger for manual external backfill (that approach is superseded — see Section 1.6's DANGER/UNTESTED mechanism, which handles this automatically as the player accumulates real matches). The flag exists so the rollout's operator can see exactly who entered without a rating and confirm the automatic mechanism is behaving correctly, not to queue up external research.
...
2. The app outputs a request, scoped strictly by tournament, not by player (see below for why). This output must be mechanically assembled directly from the app's own stored records — never generated, inferred, or written by any AI process inside the app itself. Every line in it must trace to a real, verifiable stored value. This step produces a request for information, not information — it must not itself contain any claim about a player or a result that hasn't already been verified into the store.
...
Tournament-scoped, not player-scoped — the system tracks tournaments, not players, directly. A tournament is the shell that holds match data together; a player's data only exists as a byproduct of the tournaments they've appeared in. There is no independent way to ask "what's new for this player" — only "what's new for this tournament," which then surfaces whatever player data comes with it. The request output reflects this: it is a list of tournaments, never a list of players.

For each tournament, the request states: the tournament's identity (name, tour, tier), the last known real data point on record for it (e.g., last completed round or match date), and the threshold date the fresh data must be brought up to — today's actual date. Example: if today is the 28th and a tournament's stored data runs up to February 20, the request for that tournament states its last known state (Feb 20) and asks for everything real up to the 28th.

A tournament is only included in the request if it is currently in process or already completed, relative to today's real date — never a tournament that, per its known real-world schedule, hasn't started yet. There's nothing to request for a tournament that hasn't happened.
...
If either player is UNTESTED (no established rating exists yet — Section 1.6), the page cannot compute a genuine prediction. It must state this plainly rather than fabricate a call from a number that doesn't exist.

If both players have established ratings but either carries a DANGER flag (fewer than 5 real matches on record — Section 1.6), the prediction is still computed and shown, but the DANGER marker must be surfaced visibly alongside it, not silently — the person reading it needs to know at least one side's rating is still provisional. This is a weaker condition than UNTESTED: DANGER still produces a real call, UNTESTED does not.
